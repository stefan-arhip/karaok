...
Ce composant permet à une fiche de se magnétiser à une autre fiche lors de son déplacement (comme WinAmp).
La force d'attraction est paramétrable.
Il est également possible de coller la fiche et faire déplacer la fille collée avec celle qui la colle.
------------------------------------------------------------------------------------------------------------
TSnapForm v1.3b - Composant non visuel
Correction de bugs :
  - La fiche ne se décolle plus lorsque l'on l'a déplace trop vite
  - On peut activer le collage après la magnétisation avec une prise d'effet immédiate

TSnapForm v1.2b - Composant non visuel
Améliorations :
  - Correction de bugs
  - Ajout de l'utilisation dans les MDI
  - Possibilitée de coller les bords de l'écran (de même dans les fenêtres MDI, on peut coller les bords de la surface visible)

Quelques bugs existent, en particulier :
  - Certainement dans la colle en particulier si la fiche sont collées par magnétisme et que l'on active la colle, la colle ne prend pas..
    Il faut refaire la magnétisation dans ce cas...

TSnapForm v0.9b - Composant non visuel
Quelques bugs existent, en particulier :
  - Lors d'un déplacement rapide et lorsque les fiches sont collées, la fiche se décolle
  - Le collage disparait lorsqu'on min/max la fenêtre

------------------------------------------------------------------------------------------------------------
Quelques rappels :
  - Fiche magnetique : la fiche A est attirée par la fiche B lorsque A est déplacée près de B
  - Fiche collée : la fiche A se déplace en même temps que la fiche B lorsqu'elle est déplacée

Détails des propriétés publiée :
  Active : détermine si la magnétisation est activée ou non (faux à la création) - Booléen
  ActiveStick : Détermine si on active le collage sur la fenêtre magnétisée - Booléen
  SnapStrenght : Détermine la force de la magnétisation - Integer
  SnapScreenBorder : Permet de déterminer la magnétisation sur les bords de l'écran - Booléen

Détails des fonctions :
    SetScreenRect; Enregistre la position des 4 coins de l'écran ou de la surface MDI visible dans FScreenRect
    WMMoving; Procédure apellée lors du déplacement de la fenêtre parente
    WMMovingStick; Procédure appelée lors du déplacement de la fenêtre collée
    HookStickForm; Procédure initialisant la capture du message windows de déplacement de la fenêtre collée
    UnHookStickForm; Procédure supprimant la capture du message windows de déplacement de la fenêtre collée
    HookForm; Procédure initialisant la capture du message windows de déplacement de la fenêtre parente
    UnhookForm; Procédure supprimant la capture du message windows de déplacement de la fenêtre parente
    WndProcForm(var AMsg: TMessage); Gestion des messages de la fenêtre parente
    WndProcStickForm(var AMsg: TMessage); Gestion des messages de la fenêtre collée
    SetActive(value : boolean);  Procédure activant ou non la magnétisation de la fenêtre parente
    SetStickActive(value : boolean); Procédure activant ou non le collage de la fenêtre magnétisée
    SetSnapScreenBorder(value : boolean); Procédure activant ou non le magnétisme du bord de l'écran
    create(AOwner : TCOmponent); override; Constructeur du composant
    AddFormSnap(SnapForm : string); Ajoute une fiche pour la magnétisation
    DelFormSnap(SnapForm : string); Supprime une fiche magnétisée
    Destroy; override; Destructeur
}
unit SnapForm;
...
uses
  SysUtils, Classes, Forms, Messages, windows, controls;
...
procedure Register;
begin
  RegisterComponents('Exemples', [TSnapForm]);
end;
